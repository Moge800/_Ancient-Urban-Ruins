    ...
    public class JobGiver_EnterSubMapByNeeding : ThinkNode_JobGiver
    {
        protected override Job TryGiveJob(Pawn pawn)
        {
            if (AM_ModSetting.setting.enableAICrossLevel && pawn.mindState.lastJobTag == JobTag.Idle && pawn.needs?.food != null &&
                (pawn.needs.food.CurCategory >= HungerCategory.UrgentlyHungry) && GameComponent_AncientMarket.GetComp.GetSchedule(pawn) is LevelSchedule schedule && schedule.timeSchedule[GenLocalDate.HourOfDay(pawn.Map)])
            {
